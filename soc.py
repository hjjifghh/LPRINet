# -*- coding: utf-8 -*-
"""EOC-2: sparse azimuth sampling, 17° training -> 15° real blind test."""
...
from data import EOC2_CLASSES, SARPairDataset, evaluate_clean, generate_and_save_heatmaps, gpu_sar_batch_augment, make_loader
from models import DualStreamPhysicsNet, TrainConfig, make_adamw, print_model_parameters, set_seed, train_one_epoch


def parse_args():
    parser = argparse.ArgumentParser(description="Train LPRINET on EOC-2 sparse azimuth sampling.")
    parser.add_argument("--sar-dir", default="mstar128")
    parser.add_argument("--sim-dir", default="sim128")
    parser.add_argument("--output-dir", default="output_eoc2_aligned_eoc3_400")
    ...
    parser.add_argument("--strict-sim-angle", action="store_true", help="Only inject 17° sim training samples instead of 15°+17°.")
...
def main():
    ...
    train_sim_angles = [17] if args.strict_sim_angle else [15, 17]
    results_summary = {}

    for current_ratio in ratios:
        set_seed(args.seed)
        print("\n" + "=" * 76)
        print(f"🚀 EOC-2 | 17° train -> 15° blind test | ratio={current_ratio:.2f}")
        print(f"仿真注入角度: {train_sim_angles}")
        print("=" * 76)
        ...
        train_ds = SARPairDataset(
            ...
            train_sim_angles=train_sim_angles,
        )
        ...
        for epoch in range(cfg.epochs):
            ...
            if (epoch + 1) % args.eval_interval == 0:
                acc = evaluate_clean(ema_model, test_loader, device)
                best_acc = max(best_acc, acc)
                print(f"🎯 [Ratio {current_ratio:.2f}] 15° Acc: {acc:.2f}% | best: {best_acc:.2f}%\n")
    ...
    for r, acc in results_summary.items():
        print(f"ratio={r:.2f}: best 15° Acc = {acc:.2f}%")
